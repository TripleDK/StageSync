...
class ShowListViewModel(private val showsRepository: ShowsRepository) : ViewModel() {
    val showListUiState: StateFlow<ShowListUiState> =
        showsRepository.getAllShowsStream().map { ShowListUiState(it) }
            .stateIn(
                scope = viewModelScope,
                started = SharingStarted.WhileSubscribed(TIMEOUT_MILLIS),
                initialValue = ShowListUiState()
            )
...
//                ShowListViewModel(
//                    showsRepository = stageSyncRepository
//                )
//            }
//        }
